...
@interface TagViewController () <UITextFieldDelegate, UIAlertViewDelegate, UIScrollViewDelegate, MUITagsViewDelegate>
{
    BOOL _edit;
...
- (void)done {
    ...
    if ([self.delegate respondsToSelector:@selector(tagViewController:didSelectTags:)]) {
        [self.delegate tagViewController:self didSelectTags:tags];
    }
...
- (void)setMyTags:(NSArray *)myTags {
    ...
    self.myTagsView.tags = [NSMutableArray arrayWithArray:myTags];
}
